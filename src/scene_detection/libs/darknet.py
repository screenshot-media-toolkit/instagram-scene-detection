...
def detect(net, meta, image, thresh=0.5, hier_thresh=0.5, nms=0.45, debug=False):
    """
...
def detect_image(net, meta, im, thresh=0.5, hier_thresh=0.5, nms=0.45, debug=False):
    # import cv2
    ...
    dets = get_network_boxes(
        net, im.w, im.h, thresh, hier_thresh, None, 0, pnum, letter_box
    )
    if debug:
        print("Got dets")
    num = pnum[0]
